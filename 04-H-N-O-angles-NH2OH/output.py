...
for i in list_dir:
	if fnmatch.fnmatch(i,'??????_??????_*.*'):
		...
		for j in list_config:
			with open(i + '/' + 'nh2oh_configurations' + '/' + 'H-N-O-104.746'+ '/' + 'qeq.eng') as f:
				data1 = f.readlines()
...
data1_NH3 = data1[data1['path'].str.contains('H-N-O')].sort_values(by = 'distance')
...
plt.title('H-N-O angle', fontsize = 14)
