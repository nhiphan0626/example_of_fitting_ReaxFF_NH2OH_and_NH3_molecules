...
for i in list_dir:
	if fnmatch.fnmatch(i,'??????_??????_*.*'):
		...
		for j in list_config:
			with open(i + '/' + 'nh2oh_configurations' + '/' + 'H-O-N-103.3428'+ '/' + 'qeq.eng') as f:
				data1 = f.readlines()
...
data1_NH3 = data1[data1['path'].str.contains('H-O-N')].sort_values(by = 'distance')
...
plt.xlim(65,175)
plt.ylim(-20, 150)
plt.xlabel('Valence angle (degree)', fontsize = 14)
plt.ylabel('Energy (kcal/mol)', fontsize = 14)
plt.title('H-O-N angle', fontsize = 14)
